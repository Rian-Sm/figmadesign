(() => {
  ...
  function Widget() {
    ...
    const roleOptions = Object.keys(roleCategories).map((obj) => {
      return { option: obj, label: obj };
    });
    const [role, setRole] = useSyncedState("role", roleOptions[0].option);
    const [categoriesState, setCategories] = useSyncedState("categories", roleCategories[role].categories);
    const [widthState, setWidth] = useSyncedState("width", roleCategories[role].width);
    usePropertyMenu([
      {
        itemType: "dropdown",
        propertyName: "roles",
      ...
      if (propertyName === "roles") {
        ...
        setCategories(roleCategories[propertyValue].categories);
    ...
    return /* @__PURE__ */ figma.widget.h(Frame, {
      ...
      return category.skills.map((skill, i) => {
        return Skill(skill, category.name, categoriesState, category.color, category.skillDescriptions[i], `${category.name}-${skill}`, role, showLevels, voteMap);
      });
    })), /* @__PURE__ */ figma.widget.h(AutoLayout, {
      name: "Categories",
      overflow: "visible",
      ...
      return Category(category.name, category.color, category.width, categoriesState, `${category.name}`);
    }))));
  }
  function Category(name, color, headerWidth, categoriesState, category_key) {
    const hideCategory = !categoriesState.some((item) => item == name);
  ...
  function Skill(name, category, categoriesState, color, skill_description, skill_key, role, showLevels, voteMap) {
    const offsetA = 734 - (voteMap.get(skill_key) || 1) * 130;
    const activeOpacity = 0.8;
    const hoverOpacity = 0.4;
    const hideSkill = !categoriesState.some((item) => item == category);
    return /* @__PURE__ */ figma.widget.h(Frame, {
      name: `Skill-${name}`,
      key: `Skill-${skill_key}`,
